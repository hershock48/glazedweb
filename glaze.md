...
| **glaze/ledger.md** | At the start of any session about a prospect or a client, and the moment Kevin says something was sent, answered, met or paid. One row per business with dated events and a next action; `node glaze/scripts/ledger.mjs` prints the morning digest. The data lives in `contracts-private`, not here. |
