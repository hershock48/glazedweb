...
| **glaze/ledger.md** | At the start of any session about a prospect or a client, and the moment Kevin says something was sent, answered, met or paid. One row per business with dated events, a channel and a next action; `node glaze/scripts/ledger.mjs` prints the morning digest and `node glaze/scripts/close.mjs` the closing brief with a follow-up per business. `scout.mjs` fills the prospect pool from OpenStreetMap around a town and checks each site; `select.mjs` picks the day's five inside one drive by the prospecting card. The local authority is `glazedweb-admin/data/studio.json`, reached through `contracts-private/ledger.json.authority.json`; the original ledger is archived. Session commands and the private dashboard use that same store. See ledger.md for reconciliation and the subscription-based pipeline. |
