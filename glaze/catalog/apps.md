...
|---|---|---|
| Client project page | `glazedweb/app/(en)/build/[slug]/`, `project` block per client in `lib/customOrders.js`, spec in `glaze/project-page.md` | The page a signed client opens instead of texting "any update?": five status circles (agreement, build fee and monthly live from Stripe, content collected as N of M, live on their domain), the client-facing half of the launch checklist with a WHY on every item and boxes WE tick, the sequence of what happens next with no dates until content is in, the freebies as a receipt, their links, and three lines on what the monthly is. Never restates scope; links to Exhibit A instead. Noindexed, link-only, 404s for any client without a `project` block. First client True North, 2026-09-09. |
