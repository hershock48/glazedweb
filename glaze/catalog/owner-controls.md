...
Kitchen access was additionally reviewed September 17: signed staff sessions, separate persistent attempt limits and existing owner sessions now open the board; Copper's parked menu-price editor requires owner access. See [kitchen access verification](kitchen-access-release.md). Kitchen state/order writes now use revision checks and durable receipts; pending controls lock, lost responses can be checked or retried, and owner cancellation preserves payment records. See [kitchen actions](kitchen-operations-release.md). Copper's parked menu now has revision checks, atomic before/after history, exact price drafts, pending locks and latest-copy comparison; see [parked-menu verification](copper-parked-menu-release.md). Printer confirmation and owner review now bind to a durable job and saved result; see [printer release](printer-release.md). Notification dispatch and owner recovery now use immutable messages and durable reviews; see [notification release](notification-release.md). Actual hardware, provider delivery and hosted handover remain open.
