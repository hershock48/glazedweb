...
| Booking scheduler (Calendly, owned) | `migas/lib/slots.ts` + `lib/ics.ts` (origin, TS, fullest comments) · `glazedweb/lib/schedule.js` + `app/(en)/schedule/` (live instance) | Weekly-pattern slots rendered as real radios (works scripts-off), hand-rolled RFC 5545 invites (Zulu instants, folded lines), request-a-time framing so unknown-busy is honest. The glazedweb instance is what every US-market proposal's "what happens next" links; availability pattern in `lib/schedule.js` is Kevin's to keep true. |
