...
| Sprinkles | studio `/build/sprinkles` | studio | A shop on the client's own Stripe with digital goods, so part 3 is about downloads delivered by link and files that stay hers. No try-it list until her test key is in the hosting project. Eight needs; the sugar decision (sell direct or leave it with retailers) is a need rather than a scope line, because only she can settle it. |
