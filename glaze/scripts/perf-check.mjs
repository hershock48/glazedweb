 ...
 * JavaScript under 150KB compressed. Nothing else in this kit measures any of
 * that, and the one that broke first was LCP: the Pastrami Joe's hero shipped
 * its 1600x1600 208KB source to every device and measured 3,912ms. The menu
 * page, which has no hero, measured 772ms on the same run, which is what
 * identified the image rather than the framework. Fixed with srcset to 2,084ms.
 * The CLS failure on the same build was 0.115, entirely the webfont swap.
 *
 * Measure CLS on a throttled CONNECTION, not just a throttled CPU: the anchor
 * build's 0.3947 (a server-open, client-collapsed mobile menu) only appeared
 * when the network was slow enough for the un-hydrated header to paint first.
 * This profile throttles both.
 *
 *   node glaze/scripts/perf-check.mjs --base http://127.0.0.1:4490 --routes /,/menu
 *
 ...
 *
 * Generalised from pjs/tools/perf-check.mjs: routes come from --routes instead
 * of a hard-coded list, and the browser resolves like the house auditor.
 */
